...
14. Create functions to allow users to edit/add additional information to their account, change their password, display all account information except password, and monitor system performance.
15. Create a logout function that logs the user out and redirects them to the login page.
...
### Dependencies:
The program necessitates the following Python packages(all use the latest version):
...
```
pip install getpass hashlib sqlite3 time psutil curses re email_validator unittest unittest.mock
```
...
### Usage:
Run the `main.py` file from the /src folder in a standalone terminal. Follow on-screen prompts to login/register and use other functionalities.